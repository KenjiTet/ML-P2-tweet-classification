...
def bayes_model(x_train, y_train, x_validation, y_validation, test_set):
	"""
    Trains a Naive Bayes text classification model, evaluates its accuracy on a validation set, and makes predictions on a test set.

    This function first transforms the input text data into TF-IDF vectors, then trains a Multinomial Naive Bayes classifier,
    and evaluates its performance on the validation set. Finally, it uses the trained model to make predictions on the provided test set.

    Output:
    y_subm (array-like): Predicted labels for the test_set.
	"""
...
if __name__ == "__main__":
	...
	y_submission = bayes_model(X_train, y_train, X_test, y_test, test_set)
	y_submission[y_submission == 0] = -1

	ids=np.arange(1,len(y_submission)+1)

	create_csv_submission(ids, y_submission, "predictions/pred_basic_bayes_full.csv")
